...
// @ts-ignore
import type { PreviewSubscriptionUpdateRequest } from "../model";
// @ts-ignore
...
// @ts-ignore
import type { SubscriptionUpdatePreview } from "../model";
// @ts-ignore
...
export const SubscriptionsApiAxiosParamCreator = function (configuration?: Configuration) {
  return {
    ...
    /**
     * Creates a new subscription with the specified parameters.  **Subscription Lifecycle**:  1. When first created, the subscription has a `PENDING` status 2. To initiate billing, you must call the [activate endpoint](https://docs.monei.com/apis/rest/subscriptions-activate/) with payment details 3. Once activated, the subscription will automatically bill according to the configured interval  **Key Configuration Parameters**:  - **Billing settings**: Amount, currency, interval (daily, weekly, monthly, yearly) - **Schedule customization**: Interval count, trial period duration - **Trial pricing**: Offer a free trial with `trialPeriodDays`/`trialPeriodEnd` (no charge until the trial ends), or discounted introductory pricing with `trialAmount` + `trialIntervalCount` (the `trialAmount` is charged each billing cycle for that many cycles, then the regular `amount` applies). `trialAmount` can also be combined with `trialPeriodDays`/`trialPeriodEnd` to charge it once at activation for the whole trial period. `trialIntervalCount` requires `trialAmount` and cannot be combined with `trialPeriodDays`/`trialPeriodEnd`  **Example — 5 EUR for the first 3 months, then 10 EUR/month**:  ```json {   \"amount\": 1000,   \"currency\": \"EUR\",   \"interval\": \"month\",   \"intervalCount\": 1,   \"trialAmount\": 500,   \"trialIntervalCount\": 3 } ```  The customer pays 5.00 EUR at activation and for the next two monthly cycles (3 discounted cycles in total), then 10.00 EUR every month. - **Allowed payment methods**: An array of strings specifying which payment methods are allowed for this subscription (e.g., `card`, `bizum`) - **Customer information**: Contact details, billing and shipping addresses - **Communication**: Callback URLs for webhook notifications about subscription events  **Best Practices**:  - Set clear, descriptive names for subscriptions to help with identification - Configure appropriate webhook notifications to monitor subscription status changes - Consider offering trial periods to increase customer conversion rates - Use metadata to store additional information relevant to your business logic
    ...
    /**
     * Calculate what a prorated change to a subscription would settle, without applying it.  Send the `amount`, `interval`, `intervalCount` or `nextPaymentAt` you intend to set. The response reports the credit for unused time, the charge for the new price or the extra time, and the net difference that would move. Nothing is charged, refunded or stored.  Use this to show a customer the cost of a plan change, or of moving their next charge date, before they confirm it. Then apply the change with [Update Subscription](https://docs.monei.com/apis/rest/subscriptions-update/) and `prorate: true`.  The figures are an estimate. Applying the change recalculates them at that moment, so a preview taken much earlier can differ. The same validation runs here as on the update, so a change this endpoint rejects would also be rejected when applied.
     * @summary Preview Subscription Update
     * @param {string} id The subscription ID
     * @param {PreviewSubscriptionUpdateRequest} previewSubscriptionUpdateRequest
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
     */
    preview: async (
      id: string,
      previewSubscriptionUpdateRequest: PreviewSubscriptionUpdateRequest,
      options: RawAxiosRequestConfig = {},
    ): Promise<RequestArgs> => {
      // verify required parameter 'id' is not null or undefined
      assertParamExists("preview", "id", id);
      // verify required parameter 'previewSubscriptionUpdateRequest' is not null or undefined
      assertParamExists(
        "preview",
        "previewSubscriptionUpdateRequest",
        previewSubscriptionUpdateRequest,
      );
      const localVarPath = `/subscriptions/{id}/preview`.replace(
        `{${"id"}}`,
        encodeURIComponent(String(id)),
      );
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
    ...
    /**
     * Updates the configuration of an existing subscription.  **Modifiable Parameters**:  - Billing information (amount, interval, intervalCount, description, allowedPaymentMethods) - Customer details (contact information, billing/shipping addresses) - Subscription settings (cancelAtPeriodEnd, pauseAtPeriodEnd, skipIntervalCount) - Metadata (for your internal tracking)  **Update Effects**:  - Amount changes apply to the next billing cycle - Interval changes apply from the next period, leaving the current one to run out - Setting `prorate` to true settles the price difference immediately instead, and an interval   change then restarts the billing period from now - `nextPaymentAt` moves the next charge date on its own, leaving the price alone. With   `prorate` it also settles the time that moves: later charges for it, earlier refunds it - Customer information updates take effect immediately - Setting `cancelAtPeriodEnd` to true will end the subscription after the current period - Setting `pauseAtPeriodEnd` to true will pause billing after the current period - Setting `skipIntervalCount` skips billing for the specified number of intervals without changing subscription status  **Note**: The currency cannot be changed once a subscription is created. To change it, cancel the subscription and create a new one.
...
export const SubscriptionsApiFp = function (configuration?: Configuration) {
  ...
  return {
    ...
    async preview(
      ...
      (axios?: AxiosInstance, basePath?: string) => AxiosPromise<SubscriptionUpdatePreview>
    > {
      ...
      const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
      const localVarOperationServerBasePath =
        operationServerMap["SubscriptionsApi.preview"]?.[localVarOperationServerIndex]?.url;
      return (axios, basePath) =>
        createRequestFunction(
          localVarAxiosArgs,
          globalAxios,
...
export const SubscriptionsApiFactory = function (
  ...
  return {
    ...
    preview(
      ...
      previewSubscriptionUpdateRequest: PreviewSubscriptionUpdateRequest,
      options?: RawAxiosRequestConfig,
    ): AxiosPromise<SubscriptionUpdatePreview> {
      return localVarFp
        .preview(id, previewSubscriptionUpdateRequest, options)
        .then((request) => request(axios, basePath));
    },
    /**
     * Resumes a previously paused subscription, reactivating the billing cycle.  **Resume Effects**:  - Billing is immediately reactivated - The subscription status changes from `PAUSED` to `ACTIVE` - The next billing date is recalculated based on the current date - Regular charging schedule resumes according to the subscription interval  **Important Notes**:  - No immediate charge is created when resuming (billing continues on next scheduled date) - Any pause duration is added to the current billing period, maintaining the expected number of billing cycles - If a significant time has passed, verify that the payment method is still valid
